...
local function receiveheaders(sock, headers)
    ...
    if err then
        log.error("Error getting headers " .. err)
        return nil, err
    end
    -- headers go until a blank line is found
    log.info("========Receiving headers")
    while line ~= "" do
...
function metat.__index:receivestatusline()
    local status,ec = self.try(self.c:receive(7))
    -- identify HTTP/0.9 responses, which do not contain a status line
    -- this is just a heuristic, but is what the RFC recommends
    log.info(string.format("first five bytes %s %s", status, ec))
    if status ~= "\r\nHTTP/" then
        if ec == "timeout" then
            return 408
        end 
...
--[[local]] function trequest(reqt)
    ...
    log.info("======== Sent HTTP message")
    local code, status = h:receivestatusline()
    log.info(string.format("Code: %s Status: %s", code, status))
